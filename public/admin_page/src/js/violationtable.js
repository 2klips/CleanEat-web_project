$(document).ready(function () {
    ...
    const setPage = (pageNumber) => {
        violationtable.innerHTML = ''; // 페이지 리스트를 초기화
        ...
        for (
            ...
            let violation = exData[i];
            let violationname = violation.name.length>=9?violation.name.substr(0,8)+'...':violation.name;
            let addrs = violation.addr.split(' ')
            let violationaddr = violation.addr.length>=10?addrs[0]+' '+addrs[1]+'...':violation.addr
            let detail = violation.detail.length>=9?violation.detail.substr(0,8)+'...':violation.detail;
            let penalty = violation.penalty.length>=9?violation.penalty.substr(0,8)+'...':violation.penalty;
            let newElements = `
            <tr>
                <td>${violation._id}</td>
                <td>${violation.date}</td>
                <td class="abbhover" data-name="${violation.name}">${violationname}</td>
                <td>${violation.type}</td>
                <td>${detail}</td>
                <td>${penalty}</td>
                <td class="abbhover" data-name="${violation.addr}">${violationaddr}</td>
                
            </tr>`;
            violationtable.innerHTML += newElements;
        }
    };
    ...
    $.ajax({
        url: 'api/violation', // 서버에서 사용자 데이터를 가져올 엔드포인트
        method: 'GET',
        success: function (data) {
            exData = data.violation; 
